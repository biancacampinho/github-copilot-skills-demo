...
{
    private static (ProductsController controller, Mock<ISender> mediator) CreateController()
    {
    ...
    public async Task GetAll_returns_ok_with_the_list()
    {
        ...
        IReadOnlyList<ProductDto> data = new List<ProductDto> { new() { Name = "Bluetooth headphones" } };
        mediator.Setup(m => m.Send(It.IsAny<GetProductsQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result<IReadOnlyList<ProductDto>>.Success(data));
    ...
    {
        var (controller, mediator) = CreateController();
        mediator.Setup(m => m.Send(It.IsAny<GetProductByIdQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result<ProductDto>.NotFound("not found"));
    ...
    public async Task Create_returns_created_on_success()
    {
        var (controller, mediator) = CreateController();
        var id = Guid.NewGuid();
        mediator.Setup(m => m.Send(It.IsAny<CreateProductCommand>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result<Guid>.Success(id));

        var response = await controller.Create(CreateProductCommandData.Valid());
    ...
    public async Task Update_returns_badrequest_when_route_id_differs_from_body()
    {
        ...
        var command = UpdateProductCommandData.Valid(Guid.NewGuid());
    ...
    public async Task Update_returns_nocontent_on_success()
    {
        ...
        var response = await controller.Update(id, UpdateProductCommandData.Valid(id));
