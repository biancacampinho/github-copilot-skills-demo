...
The skills used in this repo were created starting from a reusable template available at [`templates/skills-template`](./templates/skills-template), designed to standardize the structure and speed up the creation of new custom skills for GitHub Copilot.

For the demo, 3 skills were developed, designed to work in sequence on a microservice:

### `my-research`
Analyzes the microservice codebase and produces a research document describing its architecture, conventions, and main patterns. The generated document acts as context for the subsequent skills.

### `my-unit-tests`
Adds unit tests following the testing conventions already present in the repo. Measures code coverage and **never modifies files in `src/`**, ensuring that only the test layer is touched.

### `my-api-implementation`
Creates a new end-to-end endpoint, following the architectural patterns described in the context document generated by `my-research`. The result is consistent with the conventions already in use in the microservice.

---

**How they work together:** `my-research` generates the context → `my-api-implementation` uses it to create the new endpoint following existing patterns → `my-unit-tests` covers the new code with proper tests, without touching the application logic.
